...
for bar_mark, bar_parameters in barDict.items():
    ...
    # text_note = TextNote.Create(doc, view.Id, XYZ(0,endRadius,0), text, text_note_type_id)
    # text_note.TextNoteType = doc.GetElement(ElementId(1018389))
    # text_note.Coord = XYZ(text_note.Coord.X-1, text_note.Coord.Y, text_note.Coord.Z)
    # #rotate text note
    # p_1 = text_note.Coord
    ...
    rad = startRadius+spacing*2

    line1 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(pol_car(rad,DEG)[0],pol_car(rad,DEG)[1],0),XYZ(pol_car(rad,DEG+1)[0],pol_car(rad,DEG+1)[1],0)))
    line2 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(pol_car(rad+spacing,DEG)[0],pol_car(rad+spacing,DEG)[1],0),XYZ(pol_car(rad+spacing,DEG+1)[0],pol_car(rad+spacing,DEG+1)[1],0)))
    p1 = XYZ(pol_car(rad,DEG)[0],pol_car(rad,DEG)[1],0)
    p2 = XYZ(pol_car(rad+spacing,DEG)[0],pol_car(rad+spacing,DEG)[1],0)
    refArray = ReferenceArray()
    refArray.Append(line1.GeometryCurve.Reference)
    refArray.Append(line2.GeometryCurve.Reference)
    doc.Create.NewDimension(view, Line.CreateBound(p1,p2),  refArray, doc.GetElement(ElementId(1018370)))
t.Commit()
...
# #     model_line = doc.Create.NewDetailCurve(view, new_arc)
# #     model_line.LineStyle = Lstyle
# #     model_line = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(0,rad,0), XYZ(0.1,rad,0)))
# #     line1 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(0,0,0), XYZ(0,11900/304.8,0)))
# #     line2 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(0,0,0), XYZ(pol_car(11900/304.8,45)[0],pol_car(11900/304.8,45)[1],0)))
...
#     # model_line.LineStyle = Lstyle
#     # model_line = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(0,rad,0), XYZ(0.1,rad,0)))
#     line1 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(pol_car(rad,45)[0],pol_car(rad,45)[1],0)))
#     line2 = doc.Create.NewDetailCurve(view, Line.CreateBound(XYZ(pol_car(rad,45)[0],pol_car(rad,45)[1],0)))
