...
FPath =  "C:\Users\Wagner.Human\Desktop\Wolf_RebarData_RevD_V162r5.xlsx" #forms.pick_file(file_ext='xlsx', multi_file=False, unc_paths=False)
...
for i in range(5,100):
    if "PV20" in str(xl.Cells(i, 1).Value2):
        ...
        rebar_p3 = locPoint+XYZ(horOffsetInner+(barDia/2)+rTower+(ACoutRad/2)                , 0 ,  h_plinth -top_cover - barDia/2 -plinthHorConDia-r)
        rebar_p4 = locPoint+XYZ(horOffsetInner+(barDia/2)+rTower+(ACoutRad/2)+r              , 0 ,  h_plinth -top_cover - barDia/2 -plinthHorConDia)
        rebar_p5 = locPoint+XYZ(r_plinth- top_cover - plinthFaceConDia -  (barDia/2)                , 0 ,  h_plinth -top_cover - barDia/2 -plinthHorConDia-r)
        rebar_p6 = locPoint+XYZ(r_plinth- top_cover - plinthFaceConDia -  (barDia/2)                , 0 ,  bot_cover + barDia/2 )
        rebar_p7 = locPoint+XYZ(r_plinth- top_cover - plinthFaceConDia -  (barDia/2)+A              , 0 ,  bot_cover + barDia/2 )
        ...
        curve_list99j = List[Curve]([curve1, curve2, curve3, curve4, curve5])
