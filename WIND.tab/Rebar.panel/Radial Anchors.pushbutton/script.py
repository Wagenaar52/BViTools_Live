...
for i in range(5,150):
    if "RA" in str(xl.Cells(i, 1).Value2) :#!= None:
        ...
        rebarCur.LookupParameter("Mark").Set("RADIAL ANCHORS")
        ...
        if no_bars > 2:
            ...
            for elem in elem:
                doc.GetElement(elem).LookupParameter("Mark").Set("RADIAL ANCHORS")
            ...
            doc.GetElement(rebarCopy[0]).LookupParameter("Mark").Set("RADIAL ANCHORS")
            doc.GetElement(rebarCopy[0]).LookupParameter("Schedule Mark").Set(bar_mark) 
            twobarRebarCur.LookupParameter("Mark").Set("RADIAL ANCHORS")
