...
rotCount = 1
AnchorBolts = int(xl.Cells(3,4).Value2)
...
for i in range(5,100):
    if "BR" in str(xl.Cells(i, 1).Value2):
        ...
        RotSwitch = int(xl.Cells(i,5).Value2)
        StartRad = int(xl.Cells(i,2).Value2)
        EndRad = int(xl.Cells(i,3).Value2)
        StartHook = int(xl.Cells(i,11).Value2)
        
        i += 1

        print("  ----   " + "BAR MARK"+ "  ----   " + "NO BARS FACTOR" + "  ---  " + "SIZE"+ "  ---  " + "ROTATION SWITCH" + "  ---  " + "START RAD" + "  ---  " + "END RAD" + "  ---  " + "START HOOK" )
        print("  ----   " + bar_mark + "  ----   " + str(no_bars_factor) + "  ---  " + size + "  ---  " + str(RotSwitch) + "  ---  " + str(StartRad) + "  ---  " + str(EndRad) + "  ---  " + str(StartHook))
        print("*"*30)
        ...
        #Rebar Properties
        #A
        A = (h_base*2) # + (slabSlope*(h_base*2))**2)**0.5
        #B
        y_Rangetop = h_base + slabSlope*bot_cover -top_cover -barDia*1.5
        y_Rangebot = bot_cover + barDia
        B = y_Rangetop - y_Rangebot
