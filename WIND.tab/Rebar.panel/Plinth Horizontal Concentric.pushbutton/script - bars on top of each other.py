...
FPath =  "C:\Users\Wagner.Human\Desktop\Wolf_RebarData_RevD_V162r3.xlsx" 
...
for i in range(1,100):
    ...
    if "PH" in str(xl.Cells(i, 1).Value2).replace(" ","") :
        ...
        x_outerRange = r_plinth -top_cover -barDia/2
        x_innerRange = r_groutOuter + top_cover + barDia/2
        x_range = x_outerRange - x_innerRange
        print("x_outerRange: " + str(x_outerRange))
        print("x_innerRange: " + str(x_innerRange))
        print("x_range: " + str(x_range))
        no_ConRings = math.ceil((x_range)/spacing)
        print("No. of concentric rings: " + str(no_ConRings))
        spacing = x_range/(no_ConRings-1)
        print("Spacing: " + str(spacing))
        ...
        for i in range(1, int(no_ConRings)+1):
            ...
            preplane = Plane.CreateByNormalAndOrigin(XYZ.BasisZ, locPoint + XYZ(0,0,zLevel))
            ...
            midplane = Plane.CreateByThreePoints(p1,p2, locPoint+ XYZ(0,0,zLevel))
            # get normal of plane
            normal = midplane.Normal
            plane = Plane.CreateByNormalAndOrigin(normal, locPoint+ XYZ(0,0,zLevel))
